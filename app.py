...
def transform_data(ticker_df, history_dfs):
    ...
    for ticker in list(ticker_df["Ticker"]):
        ...
        for c in ["Open", "High", "Low", "Close", "Volume"]:
            history_dfs[ticker][c] = pd.to_numeric(
                history_dfs[ticker][c], errors="coerce"
            )
...
def display_symbol_history(ticker_df, history_dfs):
    ...
    with right_indicator:
        ...
        with st.container():
            ...
            avg_volume = history_df["Volume"].dropna().mean()
            st.metric(
                "Average Daily Volume",
                f"{int(avg_volume):,}" if not pd.isna(avg_volume) else "N/A",
            )
            market_cap = ticker_df[ticker_df["Ticker"] == selected_ticker][
                "Market Cap"
            ].values[0]
            st.metric(
                "Current Market Cap",
                "{:,} $".format(market_cap) if not pd.isna(market_cap) else "N/A",
            )
